...
describe("resumes_diagnostics", () => {
  ...
  describe("GET /api/resumes/search-freshness", () => {
    ...
    it("hints exit 1 when the lag scan window is incomplete and computeStale is zero", async () => {
      vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
        const url = typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
      ...
      const payload = await parseJsonBody<{
        lagScanFailed?: boolean;
        exitCodeHint?: number;
        lag?: { scanComplete?: boolean; computeStale?: number };
        messages?: string[];
      }>(response);
      expect(payload.lagScanFailed).toBe(false);
      expect(payload.lag?.scanComplete).toBe(false);
      expect(payload.lag?.computeStale).toBe(0);
      expect(payload.exitCodeHint).toBe(1);
      expect(payload.messages?.some((message) => message.includes("scanComplete=false"))).toBe(true);
    });
