...
- CPA on ptcloud (current 2026-08-28): `{WIKI_VAULT}/projects/trends/work/2026-08-28-cpa-official-upgrade-helper/` · CLIProxyAPI **7.2.144** at `/home/ubuntu/cliproxyapi` on `ssh ptcloud` · **official installer layout** (`systemctl --user` + linger; no system unit) · upgrade is `ssh ptcloud` then `cliproxyapi-installer upgrade` · Caddy `cpa.pt-mes.com` `tls internal` reverse→`127.0.0.1:8317` · no public DNS (`/etc/hosts` `217.217.255.28 cpa.pt-mes.com`) · AI routing: local `.env` `https://cpa.pt-mes.com/v1`; preview+prod `http://127.0.0.1:8317/v1` · secrets ONLY in `~/.secrets/com.trends.app.env` · host runbook `/home/ubuntu/cliproxyapi/UPGRADE-RUNBOOK.md` + repo `docs/runbooks/ptcloud-cpa.md` · original 2026-08-26 install (historical `/root` + system unit): `{WIKI_VAULT}/raw/transcripts/2026-08-26-cpa-deploy-trends-ptcloud-ai-routing.md` · **Forbidden:** recreate `/etc/systemd/system/cliproxyapi.service`; run the installer as root; disable linger for ubuntu
- Completed prod batch-reject 113 old CNC Sales MY resumes (2026-08-24): `{WIKI_VAULT}/projects/trends/work/2026-08-24-prod-batch-reject-old-cnc-sales/` · 113 Seek talentsearch MY resumes from Aug 11 export still `new` when new collection started → batch-set all to `rejected` via `POST /api/candidate-status` on prod (ptcloud, `hr` workspace, `hr-demo` admin) · identityKey computed from profile URLs matching prod v0.4.16 `normalizeProfileIdentityKey` logic (`profileUrl:hk.employer.seek.com/talentsearch/profiles/search?market=my&pagenumber=1&searchquery={name}`) · dry-run approved (113 new → rejected, 0 already rejected, 0 other) · first pass 98/113 OK, 15 rate-limited 429, retry with 3s delay 15/15 OK · verified 564→677 status records (+113 exact), all 113 confirmed `rejected` · collection STOPPED (last crawl 06:15 UTC), all 50 recent resumes from today Aug 24 + `new`, zero July/earlier mixed in · **no code changes, data-only prod operation** · key caveat: prod v0.4.16 uses name-search URL as identityKey; v0.4.21+ (`84b20e23`) rejects name-search URLs → identity falls to `externalId:...`, future batch ops on upgraded deployments must use `externalId`
...
- Windows local dev onboarding (2026-09-23): `{WIKI_VAULT}/projects/trends/work/2026-09-23-windows-dev-environment-onboarding/` · end-to-end boot on Windows 11 (host MSI) from zero: prereqs (Git Bash / Node 22 / bun / uv) → `npm install` (bun.lock version mismatch forces npm not bun) → `npm rebuild better-sqlite3` + `npm install bindings` → junction workspace bins to root hoist (convex / tsx / vite / typescript) → `bun run --filter "@trends/shared" build` → start convex/api/web independently in background (dev.sh process tree exits on Windows) → `bun run auth:bootstrap-demo` (demo-admin/admin123) → `bunx tsx scripts/seed-convex.ts --force --with-resumes` (171 resumes) → **call `resumes_search:backfillResumeDigests` mutation via Convex HTTP API** (seed writes `resumes` but search reads `resume_digests`) → search + AI analysis verified (47 CNC results, 10/10 AI analysis completed) · BFF must be restarted with `.env` loaded (`CONVEX_WRITE_SECRET`) or Convex reads return 401 · do NOT commit `bun.lock` modifications on Windows (bun 1.3.14 rewrites the committed lockfile format)
