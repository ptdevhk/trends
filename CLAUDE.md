...
- Completed CPA install + AI routing on ptcloud (2026-08-26): `{WIKI_VAULT}/raw/transcripts/2026-08-26-cpa-deploy-trends-ptcloud-ai-routing.md` · CLIProxyAPI 7.2.142 on `ssh ptcloud` (system-level `cliproxyapi.service`, `/root/cliproxyapi`), Caddy `cpa.pt-mes.com` vhost `tls internal` reverse→`127.0.0.1:8317` · no public DNS (resolve via `/etc/hosts` `217.217.255.28 cpa.pt-mes.com`) · `remote-management.allow-remote: true`, admin pw `pr0technic`, client key `sk-071be574…` (replaced installer template keys which disabled the proxy) · Poe OpenAI-compat provider `poe-lite-dd` (base `https://api.poe.com/v1`, `POE_API_KEY`, models python/deepseek-v4-flash/gemini-2.5-flash-lite/deepseek-v4-flash-e, `prefix: dd`) · AI routing: local `.env` `https://cpa.pt-mes.com/v1`; preview+prod `http://127.0.0.1:8317/v1` (ptcloud `/etc/trends/env` + `.env.preview` on host, both BFFs restarted, verified via /proc environ) · secrets ONLY in `~/.secrets/com.trends.app.env` (never in repo) · `ai-model-check.sh` PASS · **prod `AI_MODEL=openai/gpt-5.4-mini` NOT in Poe catalog — would fail if analysis enabled; switch to `openai/deepseek-v4-flash` or leave prod on Poe direct** · cpa-usage-keeper v1.14.8 deliberately SKIPPED on ptcloud · Caddy root CA at `/tmp/caddy-cpa-root.crt` on Mac (needed for orca browser trust; not yet keychain-installed)
