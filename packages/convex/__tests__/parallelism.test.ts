...
const taggingEnvCases: EnvCase[] = [
    { name: "unset", env: {} },
    { name: "non-numeric", env: { AI_TAGGING_PARALLELISM: "abc" } },
    { name: "1", env: { AI_TAGGING_PARALLELISM: "1" } },
    { name: "2", env: { AI_TAGGING_PARALLELISM: "2" } },
    { name: "4", env: { AI_TAGGING_PARALLELISM: "4" } },
    { name: "999", env: { AI_TAGGING_PARALLELISM: "999" } },
];
...
describe("resolveAiTaggingParallelism", () => {
    it("uses defaults for missing or invalid values and caps to max", () => {
        expect(resolveAiTaggingParallelism(100, {})).toBe(DEFAULT_AI_TAGGING_PARALLELISM);
        expect(resolveAiTaggingParallelism(100, { AI_TAGGING_PARALLELISM: "abc" })).toBe(DEFAULT_AI_TAGGING_PARALLELISM);
        expect(resolveAiTaggingParallelism(100, { AI_TAGGING_PARALLELISM: "0" })).toBe(DEFAULT_AI_TAGGING_PARALLELISM);
        expect(resolveAiTaggingParallelism(100, { AI_TAGGING_PARALLELISM: "-3" })).toBe(DEFAULT_AI_TAGGING_PARALLELISM);
        expect(resolveAiTaggingParallelism(100, { AI_TAGGING_PARALLELISM: "999" })).toBe(MAX_AI_TAGGING_PARALLELISM);
    });

    for (const envCase of taggingEnvCases) {
        it(`clamps and respects workload boundaries (${envCase.name})`, () => {
            let previous = 0;
            for (const workload of workloads) {
                const parallelism = resolveAiTaggingParallelism(workload, envCase.env);
                expect(parallelism).toBeGreaterThanOrEqual(1);
                expect(parallelism).toBeLessThanOrEqual(MAX_AI_TAGGING_PARALLELISM);
                if (workload > 0) {
                    expect(parallelism).toBeLessThanOrEqual(workload);
                }
                expect(parallelism).toBeGreaterThanOrEqual(previous);
...
describe("parallelism resolver stress checks", () => {
    ...
    it("maintains bounded output under large workloads (tagging)", () => {
        let previous = 0;
        for (let workload = 0; workload <= 20000; workload += 101) {
            const parallelism = resolveAiTaggingParallelism(workload, { AI_TAGGING_PARALLELISM: "999" });
            expect(parallelism).toBeGreaterThanOrEqual(1);
            expect(parallelism).toBeLessThanOrEqual(MAX_AI_TAGGING_PARALLELISM);
            if (workload > 0) {
                expect(parallelism).toBeLessThanOrEqual(workload);
    ...
    it("keeps tagging default at expected value", () => {
        expect(resolveAiTaggingParallelism(100, {})).toBe(DEFAULT_AI_TAGGING_PARALLELISM);
    });
});
