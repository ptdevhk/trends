...
The recommended snapshot is **collect-only**: it is tied to a single job posting (a time-limited,
per-position profile — per-position sessions are retained until the posting closes), so the push
script excludes `resume-backup-seek-top*.json` by default. The talentsearch snapshot (Path B) is
the seek variant that lands in `ptdevhk/trends-resume-samples`.
...
| Pushed to samples repo | No (time-limited per-position profile) | Yes |

For a full refresh that covers both markets, run both paths with different `--seek-url` values. Note that both use alias `seek`, so the second run overwrites the first's snapshot file unless you rename or use separate directories. Only the talentsearch file is pushed (see SKILL.md Push Workflow — `SNAPSHOT_EXCLUDE`).
