#!/usr/bin/env node
// Preview-only first-wave MY/TH company-industry approve.
// Mutation chain (same as restore bootstrap, additive):
...
// Reviewer: orchestration-wave1. Write secret is never printed.
...
const REVIEWER = "orchestration-wave1";
const ACK_REASON =
  "Preview-only wave1 MY/TH catalog approve: official_site evidence fetched for each employer; machineOrigin unknown unless the official page states import vs domestic.";

const planPath = process.argv[2];
if (!planPath) {
  console.error("SEED_FATAL: usage: wave1-orchestration-seed.mjs <plan.json>");
  process.exit(2);
}
...
try {
  for (const company of companies) {
    ...
    console.error(`WAVE1_STEP companyKey=${companyKey} class=${industryClass}`);
    ...
    try {
      ...
      if (
        !isAlreadyExists(err) &&
        !/not open for approval.*approved/i.test(String(err?.message ?? err))
      ) {
        throw err;
      }
    }
  ...
  console.log(
    `SEED_OK companies=${companies.length} sources=${seededSources} catalog=${(rows || []).length}`,
  );
  process.exit(0);
} catch (err) {
