#!/usr/bin/env node
// Preview-only third-wave MY/TH company-industry approve.
// Same Convex mutation chain as wave1/wave2. Reviewer: orchestration-wave3.
...
const REVIEWER = "orchestration-wave3";
const ACK_REASON =
  "Preview-only wave3 MY/TH catalog approve: official_site CNC evidence fetched for each employer; machineOrigin unknown unless the official page states import vs domestic.";

const planPath = process.argv[2];
if (!planPath) {
  console.error("SEED_FATAL: usage: wave3-orchestration-seed.mjs <plan.json>");
  process.exit(2);
}
...
try {
  for (const company of companies) {
    ...
    console.error(`WAVE3_STEP companyKey=${companyKey} class=${industryClass}`);
  ...
  console.log(
    `SEED_OK wave=3 reviewer=${REVIEWER} companies=${companies.length} approved=${approved} skippedExisting=${skippedExisting} sources=${seededSources} catalog=${(rows || []).length}`,
  );
  process.exit(0);
} catch (err) {
