...
describe("MY/TH Service Engineer search-quality harness", () => {
  ...
  it("fails aggregate-only when a market returns zero results", () => {
    const fixture = JSON.parse(readFileSync(fixturePath, "utf8")) as {
      qualityResponses: { MY: { summary?: { total?: number }; data?: unknown[] } };
    };
    fixture.qualityResponses.MY = { summary: { total: 0 }, data: [] };
    const dir = mkdtempSync(join(tmpdir(), "trends-my-th-empty-"));
