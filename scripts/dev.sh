...
start_convex() {
    ...
    # Case 2: No .env.local — auto-bootstrap a new local project.
    if [ ! -f "$convex_env_local" ]; then
        log "CONVEX" "$CYAN" "No Convex .env.local found. Bootstrapping new local project..."
        cd "$PROJECT_ROOT/packages/convex"

        local bootstrap_cmd="npx convex dev --once --configure new --dev-deployment local"
        if command -v bun >/dev/null 2>&1; then
            bootstrap_cmd="bunx convex dev --once --configure new --dev-deployment local"
        fi

        if ( set -o pipefail; $bootstrap_cmd 2>&1 | tee "$(service_log_path "convex")" ); then
            log "CONVEX" "$GREEN" "Convex project bootstrapped successfully."
        else
            log "CONVEX" "$YELLOW" "Convex bootstrap failed. Skipping Convex."
            log "CONVEX" "$YELLOW" "To set up manually: run 'npx convex dev' in packages/convex/ interactively."
            return 0
        fi
    fi

    # Case 3: .env.local exists (either pre-existing or just bootstrapped).
    if ! check_port "$port"; then
