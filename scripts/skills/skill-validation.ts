...
function parseFrontmatter(content: string): Frontmatter {
  ...
  if (typeof validationValue !== 'undefined') {
    ...
    if (typeof descriptionTermsValue !== 'undefined') {
      if (
        !Array.isArray(descriptionTermsValue) ||
        !descriptionTermsValue.every(
...
async function validateFileRule(
  repoRoot: string,
  rule: ValidationFileRule,
  label: string,
): Promise<void> {
...
export async function validateSkillFromRepo(repoRoot: string, skill: string): Promise<void> {
  const skillRoot = path.join(repoRoot, 'dev-docs', 'skills', skill);

  const requiredFiles = ['SKILL.md', path.join('agents', 'openai.yaml')];
