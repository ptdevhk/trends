...
**Finding 2 — industry review UI is unreachable for the data it reviews (FIXED, `b08c5d43`).** The web industry review pages (`industry-verification`, `industry-data`) lived under `/admin/system/settings/*`, gated by `SystemAccessGate` → `hasSystemAdminAccess` = **dev-workspace admin** only, and the system surface hardwired `workspaceRef` to `dev` (`SYSTEM_AUTH_WORKSPACE`). `hr-demo@hr` got "Admin access required" and `admin@dev` saw the empty dev workspace — no web account could review the actual HR queue (64 ready items).

Fix: `/:teamSlug/system` now routes through `WorkspaceSystemRoute` — dev system admins keep the canonical `/admin/system` redirect, and the industry surfaces are gated by a new `WorkspaceIndustryAccessGate` that accepts **admin membership of the active workspace** (the API already honors `X-Workspace-Slug`; non-industry system routes keep the dev-admin-only denial). Added `hasWorkspaceAdminAccess` + 4 route tests. **Browser-verified as hr-demo**: `/hr/system/settings/industry-verification` renders the full queue ("64 待处理提案"), per-row "解析身份" buttons appear on unmapped rows, the identity dialog shows candidates + both mapping modes (create-provisional default, existing-company with registry filter), the no-candidates state correctly suggests targeted research, the batch bar's "解析身份（N）" enables on unmapped selections — all with 0 console errors.
